...
#[no_mangle]
pub unsafe extern "C" fn wallet_test_settings_provider_save(provider: ManuallyDrop<CSettingsProvider>, value: ManuallyDrop<CTestSettings>, err: &mut ManuallyDrop<CError>) -> bool {
    handle_exception_result(|| {
        let provider = provider.as_ref::<Arc<SettingsProvider>>()?;
